...
if(isset($_POST['verify'])){
	...
	$checkUserQ = "SELECT `id`, `password` FROM `".$tblPrefix."users` WHERE email = '$email' AND status>0";
	...
	if(mysqli_num_rows($checkUser) >= 1){
		...
		$fallbackUrl = SITE_URL.'/admin/reset-pass.php?u='.$encMail.'&token='.$token;

		if(smtp_mailer($email, $subject, $fallbackUrl, 2)==true){
	...
	$password=mysqli_real_escape_string($conn,ps_secure_string($_POST['password']));
	$Cnfpassword=mysqli_real_escape_string($conn,ps_secure_string($_POST['conf-password']));
	if($password == $Cnfpassword){
		...
		if(mysqli_num_rows($checkUser) == 1){
			...
			if (hash_equals($user['token'], $mac)) {
				$updatePass = mysqli_query($conn, "UPDATE `".$tblPrefix."users` SET password = '$newPass', token = '' WHERE id = ".$user['user_id']);
				if ($updatePass==true){
					unset($_SESSION['user']);
		...
		header('location:admin/reset-pass.php?u='.$email.'&token='.$token);
		exit();
	}
}
